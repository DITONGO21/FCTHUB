...
CREATE TABLE public.profiles (
    ...
    name TEXT NOT NULL,
    email TEXT,
    role TEXT NOT NULL DEFAULT 'aluno' CHECK (
        role IN ('aluno', 'professor', 'admin')
    ),
...
-- 2b. TURMA_PROFESSORS (relação N:N entre turmas e professores)
CREATE TABLE public.turma_professors (
...
CREATE POLICY "turmas_select" ON public.turmas FOR
SELECT USING (true);

CREATE POLICY "turmas_insert" ON public.turmas FOR
INSERT
WITH
    CHECK (
        (
...
CREATE POLICY "turmas_delete" ON public.turmas FOR DELETE USING (
    (
        SELECT role
        FROM public.profiles
        WHERE
...
-- turma_professors: todos veem, prof/admin gerem
CREATE POLICY "turma_professors_select" ON public.turma_professors FOR
SELECT USING (true);

CREATE POLICY "turma_professors_insert" ON public.turma_professors FOR
INSERT WITH CHECK (
    (SELECT role FROM public.profiles WHERE id = auth.uid()) IN ('professor', 'admin')
);

CREATE POLICY "turma_professors_delete" ON public.turma_professors FOR
DELETE USING (
    (SELECT role FROM public.profiles WHERE id = auth.uid()) IN ('professor', 'admin')
);

-- projects: todos veem; alunos criam os seus; prof/admin atualizam
CREATE POLICY "projects_select" ON public.projects FOR
SELECT USING (true);

CREATE POLICY "projects_insert" ON public.projects FOR
INSERT
WITH
    CHECK (auth.uid () = author_id);

CREATE POLICY "projects_update" ON public.projects FOR
UPDATE USING (
    auth.uid () = author_id
    OR (
        SELECT role
...
-- versions
CREATE POLICY "versions_select" ON public.versions FOR
SELECT USING (true);

CREATE POLICY "versions_insert" ON public.versions FOR
INSERT
WITH
    CHECK (auth.uid () = author_id);

CREATE POLICY "versions_update" ON public.versions FOR
UPDATE USING (
    (
        SELECT role
        FROM public.profiles
...
CREATE POLICY "files_insert" ON public.files FOR
INSERT
WITH
    CHECK (auth.uid () = author_id);
...
CREATE POLICY "notif_select" ON public.notifications FOR
...
CREATE POLICY "notif_update" ON public.notifications FOR
UPDATE USING (auth.uid () = user_id);

-- events
CREATE POLICY "events_select" ON public.events FOR
SELECT USING (true);

CREATE POLICY "events_insert" ON public.events FOR
INSERT
WITH
    CHECK (
        (
...
-- activity
CREATE POLICY "activity_select" ON public.activity FOR
SELECT USING (true);

CREATE POLICY "activity_insert" ON public.activity FOR
INSERT
WITH
    CHECK (true);
